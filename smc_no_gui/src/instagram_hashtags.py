...
def get_instagram_posts(keyword, ig_creds):
    ...
    f = open(directory+'/%s_instagram_posts.csv' % keyword, 'w')
    w = csv.writer(f)
    ...
    instagram = pd.read_csv(directory+'/%s_instagram_posts.csv' % keyword)
    instagram[instagram['time_crawled'].notnull()].to_csv(directory+'/%s_instagram_posts.csv' % keyword, index=False)
