...
def facebook_posts_header(keyword):
    with open('_data/%s_facebook_statuses.csv' % keyword, 'w') as file:
        w = csv.writer(file)
        w.writerow(['status_id', 'status_message', 'link_name', 'status_type', 'status_link',
                    'status_published', 'num_likes', 'num_comments', 'num_shares',

                    'comments_data','comments_summary_can_comment','comments_summary_order',
                    'likes_paging','likes_summary_can_like','likes_summary_has_liked',

                    'time_crawled'])


def facebook_posts_crawler(page_id, access_token, keyword):
    ...
    def scrapeFacebookPageFeedStatus(page_id, access_token):
        with open('_data/%s_facebook_statuses.csv' % keyword, 'a') as file:
            ...
            print( "Scraping %s Facebook Page: %s" % (page_id, scrape_starttime) )
            try:
            ...
            while has_next_page:
                ...
                if 'paging' in statuses.keys():
                    ...
                    has_next_page = False
            print( "Done! %s Statuses Processed in %s\n" % (num_processed, datetime.datetime.now() - scrape_starttime) )
